...
github_repo="${CRYPTA_GITHUB_REPOSITORY:-imeelinew/Crypta}"
team_id="${CRYPTA_TEAM_ID:-5Q5QT76MJU}"
work_dir=$(mktemp -d /tmp/crypta-release.XXXXXX)
...
git add Crypta.xcodeproj/project.pbxproj
if ! git diff --cached --quiet; then
    git commit -m "Release $tag"
fi

archive_path="$work_dir/Crypta.xcarchive"
derived_data="$work_dir/DerivedData"
xcodebuild archive \
    -project Crypta.xcodeproj \
    -scheme Crypta \
...
app_path="$archive_path/Products/Applications/Crypta.app"
[[ -d "$app_path" ]] || {
...
archive_file="$work_dir/Crypta-${version}.zip"
